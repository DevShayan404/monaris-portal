import { Component } from '@angular/core';
import { NzTableModule } from 'ng-zorro-antd/table';
import { NzInputModule } from 'ng-zorro-antd/input';
import { NzDividerModule } from 'ng-zorro-antd/divider';
import { MerchantService } from '../../../../../core/services/merchant/merchant.service';
import { NzPaginationModule } from 'ng-zorro-antd/pagination';
import { FormsModule } from '@angular/forms';
import { SearchPipe } from '../../../../../core/pipe/search.pipe';
import pdfMake from 'pdfmake/build/pdfmake';
import pdfFonts from 'pdfmake/build/vfs_fonts';
import { CommonModule, DatePipe } from '@angular/common';
// Assign the virtual file system (vfs) for fonts
if (pdfFonts.pdfMake && pdfFonts.pdfMake.vfs) {
  pdfMake.vfs = pdfFonts.pdfMake.vfs;
} else {
  console.error('pdfFonts.pdfMake.vfs is undefined or null');
}
import { NzDrawerModule } from 'ng-zorro-antd/drawer';
import { NzDescriptionsModule } from 'ng-zorro-antd/descriptions';
import { NzSpinModule } from 'ng-zorro-antd/spin';

@Component({
  selector: 'app-view-merchants',
  standalone: true,
  imports: [
    NzTableModule,
    NzInputModule,
    NzDividerModule,
    NzPaginationModule,
    FormsModule,
    SearchPipe,
    NzDrawerModule,
    NzDescriptionsModule,
    NzSpinModule,
    CommonModule,
  ],
  templateUrl: './view-merchants.component.html',
  styleUrl: './view-merchants.component.css',
  providers: [DatePipe],
})
export class ViewMerchantsComponent {
  tableList!: any[];
  loading: boolean = false;
  searchKeyword: string = '';
  currentDate: any;
  visible: boolean = false;
  spinner: boolean[] = [];
  merchantDetail: any[] = [];
  constructor(private service: MerchantService, private datePipe: DatePipe) {
    this.currentDate = this.datePipe.transform(new Date(), 'dd/MM/yyyy');
  }

  ngOnInit(): void {
    this.loading = true;
    this.service.getAllMerchantList().subscribe({
      next: (res) => {
        this.tableList = res?.data?.getAllMerchants;
        // console.log(this.tableList);
        this.loading = false;
      },
      error: (err) => {
        this.loading = false;
      },
    });
  }
  viewMerchantDetail(id: number, index: number): void {
    this.spinner[index] = true;
    this.service.getMerchantDetail(id).subscribe({
      next: (res) => {
        this.merchantDetail = [
          {
            chainDetails: [
              {
                firstName: JSON.parse(res.securityDetails).SecurityFirstName,
                lastName: JSON.parse(res.securityDetails).SecurityLastName,
                phone: JSON.parse(res.securityDetails).Telephone,
                chainId: JSON.parse(res.chainDetails).merchantchainId,
              },
            ],
          },
          {
            bankingDetails: [
              {
                currency: JSON.parse(res.bankDetails).CurrencyCode,
                institution: JSON.parse(res.bankDetails).BankInstitution,
                transit: JSON.parse(res.bankDetails).BankTransit,
                accountNo: JSON.parse(res.bankDetails).BankAccountNumber,
              },
            ],
          },
          {
            businessDetails: [
              {
                mastercard: JSON.parse(res.businessDetails).MasterCardVolume,
                visacard: JSON.parse(res.businessDetails).VisaVolume,
                discover: JSON.parse(res.businessDetails).DiscoverVolume,
                unionPay: JSON.parse(res.businessDetails).UnionPayVolume,
                interac: JSON.parse(res.businessDetails).InteracVolume,
                amex: JSON.parse(res.businessDetails).AmexVolume,
                mcc: JSON.parse(res.businessDetails).MccCode,
                avgTicketZise: JSON.parse(res.businessDetails).AvgTicketSize,
                maxTransaction: JSON.parse(res.businessDetails).MaxTransaction,
                directSelling: JSON.parse(res.businessDetails).DirectSelling,
                eCommerce: JSON.parse(res.businessDetails).Ecommerce,
                moto: JSON.parse(res.businessDetails).Moto,
                businessUrl: JSON.parse(res.businessDetails).BusinessURL,
                serviceDeliverToCanada: JSON.parse(res.businessDetails)
                  .ServiceDeliverToCanada,
                serviceDeliverToUS: JSON.parse(res.businessDetails)
                  .ServiceDeliverToUS,
                isSeasonalNerchant: JSON.parse(res.businessDetails)
                  .IsSeasonalMerchant,
              },
            ],
          },
          {
            merchantDetail: [
              {
                legalName: JSON.parse(res.chainDetails).legalName,
                email: JSON.parse(res.chainDetails).email,
                operatingAsName: JSON.parse(res.chainDetails).OperatingAsName,
                language: JSON.parse(res.chainDetails).language,
                cardHolderName: JSON.parse(res.chainDetails).cardHolderName,
                yearsOfOwnership: JSON.parse(res.chainDetails)
                  .yearsOfOwernership,
                monthOfOwnership: JSON.parse(res.chainDetails).monthOfOwnerShip,
                customerServicePhoneNo: JSON.parse(res.chainDetails)
                  .customerServicePhoneNumber,
              },
            ],
          },
          {
            merchantLocation: [
              {
                locationTypeId: JSON.parse(res.merchantLocation).LocationTypeId,
                telephone: JSON.parse(res.merchantLocation).Telephone,
                country: JSON.parse(res.merchantLocation).Country,
                province: JSON.parse(res.merchantLocation).Province,
                city: JSON.parse(res.merchantLocation).City,
                postalCode: JSON.parse(res.merchantLocation).PostalCode,
                addressNo: JSON.parse(res.merchantLocation).AddressNumber,
                street: JSON.parse(res.merchantLocation).StreetName,
              },
            ],
          },
          {
            businessActivity: [
              {
                dropShipping: JSON.parse(res.businessActivity).dropShipping,
              },
            ],
          },
          {
            securityDetail: [
              {
                firstName: JSON.parse(res.securityDetails).SecurityFirstName,
                lastName: JSON.parse(res.securityDetails).SecurityLastName,
                birthDate: JSON.parse(res.securityDetails).BirthDate,
                allowCreditReports: JSON.parse(res.securityDetails)
                  .AllowCreditReport,
                percentageOfOwnership: JSON.parse(res.securityDetails)
                  .PercentageOwnership,
                title: JSON.parse(res.securityDetails).Title,
                typeOfBusiness: JSON.parse(res.securityDetails).TypeOfBusiness,
              },
            ],
          },
        ];
        console.log(this.merchantDetail);

        this.visible = true;
        this.spinner[index] = false;
      },
    });
  }

  close(): void {
    this.visible = false;
  }

  generatePDF() {
    const testImageDataUrl =
      'data:image/png;base64,';
    const documentDefinition = {
      content: [
        {
          text: this.currentDate,
          absolutePosition: { x: 500, y: 20 },
          fontSize: 9,
        },
        {
          table: {
            widths: ['*'],
            body: [
              [
                {
                  image: testImageDataUrl,
                  fit: [100, 100] as [number, number],
                  alignment: 'center',
                  margin: [0, 0, 0, 20] as [number, number, number, number],
                  border: [false, false, false, false],
                },
              ],
            ],
          },
        },
        {
          text: 'Merchant Records:',
          fontSize: 16,
          bold: true,
          margin: [0, 0, 0, 10] as [number, number, number, number],
        },
        {
          table: {
            headerRows: 1,
            widths: ['auto', 'auto', 'auto', 'auto', 'auto', '*'],
            body: [
              [
                {
                  text: 'MID',
                  style: 'tableHeader',
                  fillColor: '#fafafa',
                  border: [false, false, false, true],
                  alignment: 'center',
                  margin: [0, 8, 0, 8] as [number, number, number, number],
                  borderColor: ['#d7d7d7', '#d7d7d7', '#d7d7d7', '#d7d7d7'],
                  fontSize: 9,
                },
                {
                  text: 'Chain Id',
                  style: 'tableHeader',
                  fillColor: '#fafafa',
                  border: [false, false, false, true],
                  alignment: 'center',
                  margin: [0, 8, 0, 8] as [number, number, number, number],
                  borderColor: ['#d7d7d7', '#d7d7d7', '#d7d7d7', '#d7d7d7'],
                  fontSize: 9,
                },
                {
                  text: 'Legal Name',
                  style: 'tableHeader',
                  fillColor: '#fafafa',
                  border: [false, false, false, true],
                  alignment: 'center',
                  margin: [0, 8, 0, 8] as [number, number, number, number],
                  borderColor: ['#d7d7d7', '#d7d7d7', '#d7d7d7', '#d7d7d7'],
                  fontSize: 9,
                },
                {
                  text: 'E-mail',
                  style: 'tableHeader',
                  fillColor: '#fafafa',
                  border: [false, false, false, true],
                  alignment: 'center',
                  margin: [0, 8, 0, 8] as [number, number, number, number],
                  borderColor: ['#d7d7d7', '#d7d7d7', '#d7d7d7', '#d7d7d7'],
                  fontSize: 9,
                },
                {
                  text: 'Phone no',
                  style: 'tableHeader',
                  fillColor: '#fafafa',
                  border: [false, false, false, true],
                  alignment: 'center',
                  margin: [0, 8, 0, 8] as [number, number, number, number],
                  borderColor: ['#d7d7d7', '#d7d7d7', '#d7d7d7', '#d7d7d7'],
                  fontSize: 9,
                },
                {
                  text: 'Operating as Name',
                  style: 'tableHeader',
                  fillColor: '#fafafa',
                  border: [false, false, false, true],
                  alignment: 'center',
                  margin: [0, 8, 0, 8] as [number, number, number, number],
                  borderColor: ['#d7d7d7', '#d7d7d7', '#d7d7d7', '#d7d7d7'],
                  fontSize: 9,
                },
              ],
              ...this.tableList.map((data) => [
                {
                  text: data.merchantID,
                  color: '#6c757d',
                  border: [false, false, false, true],
                  borderColor: ['#f0f0f0', '#f0f0f0', '#f0f0f0', '#f0f0f0'],
                  alignment: 'center',
                  margin: [0, 8, 0, 8] as [number, number, number, number],
                  fontSize: 9,
                },
                {
                  text: data.chainID,
                  color: '#6c757d',
                  border: [false, false, false, true],
                  borderColor: ['#f0f0f0', '#f0f0f0', '#f0f0f0', '#f0f0f0'],
                  alignment: 'center',
                  margin: [0, 8, 0, 8] as [number, number, number, number],
                  fontSize: 9,
                },
                {
                  text: data.legalName || '-',
                  color: '#6c757d',
                  border: [false, false, false, true],
                  borderColor: ['#f0f0f0', '#f0f0f0', '#f0f0f0', '#f0f0f0'],
                  alignment: 'center',
                  margin: [0, 8, 0, 8] as [number, number, number, number],
                  fontSize: 9,
                },
                {
                  text: data.email || '-',
                  color: '#6c757d',
                  border: [false, false, false, true],
                  borderColor: ['#f0f0f0', '#f0f0f0', '#f0f0f0', '#f0f0f0'],
                  alignment: 'center',
                  margin: [0, 8, 0, 8] as [number, number, number, number],
                  fontSize: 9,
                },
                {
                  text: data.phoneNumber || '-',
                  color: '#6c757d',
                  border: [false, false, false, true],
                  borderColor: ['#f0f0f0', '#f0f0f0', '#f0f0f0', '#f0f0f0'],
                  alignment: 'center',
                  margin: [0, 8, 0, 8] as [number, number, number, number],
                  fontSize: 9,
                },
                {
                  text: data.operatingName || '-',
                  color: '#6c757d',
                  border: [false, false, false, true],
                  borderColor: ['#f0f0f0', '#f0f0f0', '#f0f0f0', '#f0f0f0'],
                  alignment: 'center',
                  margin: [0, 8, 0, 8] as [number, number, number, number],
                  fontSize: 9,
                },
              ]),
            ],
          },
        },
      ],
    };
    pdfMake.createPdf(documentDefinition).open();
  }

  generateDetailPDF() {
    const testImageDataUrl =
      'data:image/png;base64,';
    const documentDefinition = {
      content: [
        {
          text: this.currentDate,
          absolutePosition: { x: 500, y: 20 },
          fontSize: 9,
        },
        {
          table: {
            widths: ['*'],
            body: [
              [
                {
                  image: testImageDataUrl,
                  fit: [100, 100] as [number, number],
                  alignment: 'center',
                  margin: [0, 0, 0, 20] as [number, number, number, number],
                  border: [false, false, false, false],
                },
              ],
            ],
          },
        },
        {
          text: 'Merchant Details:',
          fontSize: 16,
          bold: true,
          margin: [0, 0, 0, 10] as [number, number, number, number],
        },
        {
          table: {
            widths: ['*'],
            body: [
              [
                {
                  text: 'Chain Details',
                  style: 'tableHeader',
                  fillColor: '#fafafa',
                  border: [false, false, false, true],
                  margin: [0, 5, 0, 5] as [number, number, number, number],
                  borderColor: ['#d7d7d7', '#d7d7d7', '#d7d7d7', '#d7d7d7'],
                  fontSize: 10,
                },
              ],
            ],
          },
        },
        {
          columns: [
            ...this.merchantDetail[0]?.chainDetails.map((data: any) => [
              {
                width: '*',
                text: 'First Name:' + ' ' + data.firstName,
                fontSize: 9,
                margin: [4, 10, 0, 0] as [number, number, number, number],
              },
              {
                width: '*',
                text: 'First Name:' + ' ' + data.firstName,
                fontSize: 9,
                margin: [4, 10, 0, 0] as [number, number, number, number],
              },
              {
                width: '*',
                text: 'First Name:' + ' ' + data.firstName,
                fontSize: 9,
                margin: [4, 10, 0, 0] as [number, number, number, number],
              },
              {
                width: '*',
                text: 'First Name:' + ' ' + data.firstName,
                fontSize: 9,
                margin: [4, 10, 0, 0] as [number, number, number, number],
              },
            ]),
          ],
        },
        {
          columns: this.merchantDetail
            .map((merchant) => {
              const details = merchant.chainDetails;
              return [
                {
                  width: '*',
                  text: 'First Name: ' + details,
                  fontSize: 9,
                  margin: [4, 10, 0, 0],
                },
                // {
                //   width: '*',
                //   text: 'Last Name: ' + details.lastName,
                //   fontSize: 9,
                //   margin: [4, 10, 0, 0],
                // },
                // {
                //   width: '*',
                //   text: 'Phone: ' + details.phone,
                //   fontSize: 9,
                //   margin: [4, 10, 0, 0],
                // },
                // {
                //   width: '*',
                //   text: 'Chain ID: ' + details.chainId,
                //   fontSize: 9,
                //   margin: [4, 10, 0, 0],
                // },
              ];
            })
            .flat(),
        },
      ],
    };
    pdfMake.createPdf(documentDefinition).open();
  }
}
